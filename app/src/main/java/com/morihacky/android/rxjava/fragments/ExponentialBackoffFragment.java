...
public class ExponentialBackoffFragment extends BaseFragment {
  ...
  @OnClick(R.id.btn_eb_retry) public void startRetryingWithExponentialBackoffStrategy() {
    _logs = new ArrayList<>();
    _adapter.clear();

    DisposableSubscriber<Object> disposableSubscriber = new DisposableSubscriber<Object>() {
      @Override public void onNext(Object aVoid) {
        Timber.d("on Next");
      }
  ...
  @OnClick(R.id.btn_eb_delay) public void startExecutingWithExponentialBackoffDelay() {
    ...
    Flowable.range(1, 4)
        .delay(integer -> {
          ...
          return RxJavaInterop.toV2Flowable(
              ...
              .flatMap(targetSecondDelay -> Flowable.just(integer)
                  .delay(targetSecondDelay, TimeUnit.SECONDS));
        })
        .doOnSubscribe(s -> _log(
            String.format("Execute 4 tasks with delay - time now: [xx:%02d]", _getSecondHand())))
        .subscribe(disposableSubscriber);
  ...
  private int _getSecondHand() {
    ...
    return (int) (TimeUnit.MILLISECONDS.toSeconds(millis) - TimeUnit.MINUTES.toSeconds(
  ...
  public class RetryWithDelay implements Function<Flowable<? extends Throwable>, Publisher<?>> {
    ...
    @Override public Publisher<?> apply(Flowable<? extends Throwable> inputObservable) {
      ...
      return inputObservable.flatMap(new Function<Throwable, Publisher<?>>() {
        @Override public Publisher<?> apply(Throwable throwable) {
