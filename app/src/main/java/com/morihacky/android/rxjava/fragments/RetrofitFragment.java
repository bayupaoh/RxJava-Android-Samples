...
public class RetrofitFragment extends Fragment {
  ...
  @OnClick(R.id.btn_demo_retrofit_contributors) public void onListContributorsClicked() {
    _adapter.clear();
            ...
            .subscribeWith(new DisposableObserver<List<Contributor>>() {
              ...
              @Override public void onNext(List<Contributor> contributors) {
                for (Contributor c : contributors) {
                  _adapter.add(
                      format("%s has made %d contributions to %s", c.login, c.contributions,
                          _repo.getText().toString()));

                  Timber.d("%s has made %d contributions to %s", c.login, c.contributions,
                      _repo.getText().toString());
                }
              }
            }));
    ...
    _disposables.add(
        _githubService.contributors(_username.getText().toString(), _repo.getText().toString())
            ...
            .subscribeWith(new DisposableObserver<Pair<User, Contributor>>() {
              ...
              @Override public void onNext(Pair pair) {
                User user = ((Pair<User, Contributor>) pair).first;
                Contributor contributor = ((Pair<User, Contributor>) pair).second;

                _adapter.add(format("%s(%s) has made %d contributions to %s", user.name, user.email,
                    contributor.contributions, _repo.getText().toString()));

                _adapter.notifyDataSetChanged();

                Timber.d("%s(%s) has made %d contributions to %s", user.name, user.email,
                    contributor.contributions, _repo.getText().toString());
              }
            }));
  }
