...
public class PollingFragment extends BaseFragment {
  ...
  @OnClick(R.id.btn_start_simple_polling) public void onStartSimplePollingClicked() {
    ...
    Disposable d = Flowable.interval(INITIAL_DELAY, POLLING_INTERVAL, TimeUnit.MILLISECONDS)
        ...
        .subscribe(taskName -> {
          _log(String.format(Locale.US, "Executing polled task [%s] now time : [xx:%02d]", taskName,
              _getSecondHand()));
        });
  ...
  public void onStartIncreasinglyDelayedPolling() {
    ...
    _disposables.add(Flowable.just(1L)
        .repeatWhen(new RepeatWithDelay(pollCount, pollingInterval))
        .subscribe(o -> _log(String.format(Locale.US, "Executing polled task now time : [xx:%02d]",
            _getSecondHand())), e -> Timber.d(e, "arrrr. Error")));
  }
  ...
  public class RepeatWithDelay implements Function<Flowable<Object>, Publisher<Long>> {
    ...
    @Override public Publisher<Long> apply(Flowable<Object> inputFlowable) throws Exception {
      ...
      return inputFlowable.flatMap(new Function<Object, Publisher<Long>>() {
        @Override public Publisher<Long> apply(Object o) throws Exception {
